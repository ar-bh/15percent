...
# have to reproduce the default types because we can't examine built-in enums
enum TYPE {
	NIL = 0,
	BOOL = 1,
	INT = 2,
		...
		func() -> String:
			...
			for index in fixed_data.item_count:
				...
				if not (value is Resource):
					return tr("Item %s in the array is not of the correct type. There should be only PoweredItem elements") % [index]
			return ""
	)
	_add_callable_requirement(
		...
		get_tree().process_frame.connect(
			func():
				...
				for power_idx in powerups_count:
					...
					if power_texture.texture == null or power_texture.texture != texture:
...
func _build_checks() -> void:
	...
	check_powerups_are_displayed.checker = func() -> String:
		...
		if not fixed_data.are_correct_textures_displayed:
			return tr("It seems the icon isn't being set correctly. Did you assign the power's 'image' property to the TextureRect's 'texture' property?")
		return ""
	...
	check_buttons_equal_dictionary.checker = func() -> String:
		if child_count != fixed_data.item_count:
			return tr("The array has %s items, while there are %s buttons created") % [fixed_data.item_count, child_count]
		return ""
