...
func _build_checks() -> void:
	...
	check_buttons_equal_dictionary.checker = func() -> String:
		...
		if child_count != items_list.size():
			return tr("The array has %s items, while there are %s buttons created")%[items_list.size(), child_count]
		return ""
		...
		for child_idx in child_count:
			...
			for property_name in ['text', 'amount', 'price']:
				...
				if button[property_name] != entry[property_name]:
					return tr("Button #%s has '%s' set to '%s', but the corresponding entry has it set to '%s'. Did you forget to set the property?")%[child_idx, property_name, button[property_name], entry[property_name]]
		return ""
