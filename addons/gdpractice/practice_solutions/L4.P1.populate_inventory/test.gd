...
func _build_requirements() -> void:
	...
	_add_callable_requirement(
		"There should be items in the 'items_list' dictionary",
		func() -> String:
			if _practice.items_list.size() < 1:
				return tr("There are no items in the 'items_list' dictionary. Having some items is required.")
			return ""
	)
	_add_callable_requirement(
		...
		func() -> String:
			for index in _practice.items_list.size():
				...
				if not (value is int):
					return tr("The 'items_list' dictionary value '%s' isn't an int.")%[key]
			return ""
	)
	_add_callable_requirement(
...
func _build_checks() -> void:
	...
	check_buttons_equal_dictionary.checker = func() -> String:
		if child_count != items_list.size():
			return tr("The dictionary has %s keys, while there are %s buttons created")%[items_list.size(), child_count]
		return ""
	...
	check_buttons_keys.checker = func() -> String:
		...
		for child_idx in child_count:
			...
			if button.text != key:
				return tr("The key #%s is '%s', but the button's text is '%s'")%[child_idx, key, button.text if button.text != "" else "null"]
		return ""

	var check_buttons_values := Check.new()
	check_buttons_values.description = tr("Button's amount should equal the dictionary's values")
	...
	check_buttons_values.checker = func() -> String:
		...
		for child_idx in child_count:
			...
			if button.amount != value:
				return tr("The value #%s is '%s', but the button's amount is set to '%s'")%[child_idx, value, button.amount if button.amount != 0 else "null"]
		return ""
