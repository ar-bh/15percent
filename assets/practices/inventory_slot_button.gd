## A button that displays an inventory item with its name, icon, quantity, and
## price.
##
## You create one instance per item in your dictionary, then set the [member text]
## and [member amount] properties to display the item name and quantity. The icon
## and colors are handled automatically based on the item name.
##
## Example:
## [codeblock]
...
## button.text = "sword"
## button.amount = 1
## some_container.add_child(button)
## [/codeblock]
@tool
class_name InventorySlotButton extends PanelContainer

## Emitted when the player clicks the button.
signal pressed
...
## The name of the item, displayed at the top of the button. Setting this also
## updates the displayed icon automatically.
@export var text := "": set = set_text
## The quantity of the item, displayed at the bottom left of the button. The
## quantity is hidden when set to zero.
@export var amount := 0: set = set_amount
## The price of the item, displayed at the bottom right of the button next to a
## coin icon. The price is hidden when set to zero.
@export var price := 0: set = set_price

## The icon displayed in the center of the button. This is set automatically when
## you assign [member text], so you normally do not need to set it yourself.
var texture: Texture = null: set = set_texture

var _colors_list: Array[Color] = [
	Color.html("10B2EF"),
	Color.html("FF417D"),
	Color.html("FFD500"),
	Color.html("9FDD51")
...
func _init() -> void:
	...
	_texture_rect.modulate = _colors_list.pick_random()
...
## Sets the icon texture and picks a new random tint color.
...
## Sets the item name, updates the label, and loads the matching icon from [PracticeIcons].
...
## Sets the item quantity and updates the quantity label. Hides the quantity display when set to zero.
...
## Sets the item price and updates the price label. Hides the price display when set to zero.
...
## Shows or hides the labels and icons at the bottom of the button based on whether amount and price are greater than zero.
